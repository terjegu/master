...
\begin{definition}
	\begin{equation}
		...
		\begin{split}
			\ita(\mathbf{a},\mathbf{b}) = & \log\parant{\frac{\mathbf{b}^T \ttt(\auto_{a}) \mathbf{b}}{\mathbf{a}^T \ttt(\auto_{aa}) \mathbf{a}}} \\
			 = & \log\parant{\mathbf{b}^T \ttt(\auto_{a}) \mathbf{b}}, \quad a_1 = 1
		\end{split}
	\end{equation}
\end{definition}
...
\begin{equation}
	\mathbf{D}(i,j) = \log\parant{\mathbf{b}_j^T \ttt(\auto_{a_i}) \mathbf{b}_j}
\end{equation}
