...
Reflection coefficients (RC\abbrev{RC}{Reflection Coefficients}) is another equivalent representation of LPC which could easily be checked for stability. The reflection coefficients has a dynamic range of $\sbrackets{-1,1}$ if they are stable. 
...
A metric for relative improvement from the source pitch $f_0^s$ used in this paper is the normalised pitch distance (NPD\abbrev{NPD}{Normalised Pitch Distance}) \cite{najjary04}
\begin{equation}
